...
import type { RunView, WorkshopState } from '../../src/shared/reportTypes.ts';

/** Workshop control helpers for browser tests. They call the same /api/workshop routes the panel uses. */
export async function reset(request: APIRequestContext): Promise<void> {
  const res = await request.post('/api/workshop/reset', { data: {} });
...
export async function config(request: APIRequestContext, body: { mode?: 'automatic' | 'manual'; failNextGeneration?: boolean }): Promise<void> {
  const res = await request.post('/api/workshop/config', { data: body });
  expect(res.ok(), await res.text()).toBe(true);
}

export async function state(request: APIRequestContext): Promise<WorkshopState> {
  const res = await request.get('/api/workshop/state');
  expect(res.ok()).toBe(true);
  return (await res.json()) as WorkshopState;
}
...
export async function begin(request: APIRequestContext, runId: string): Promise<RunView> {
  const res = await request.post(`/api/workshop/runs/${runId}/begin`, { data: {} });
  expect(res.ok(), await res.text()).toBe(true);
...
export async function finish(request: APIRequestContext, runId: string): Promise<RunView> {
  const res = await request.post(`/api/workshop/runs/${runId}/finish`, { data: {} });
  expect(res.ok(), await res.text()).toBe(true);
  return ((await res.json()) as { run: RunView }).run;
}
